...
Le Token Service Provider doit en plus de la dé-tokenisation normale effecté des vérification complémentaire pour etre sur que le Token Domain Restriction Controls est bien addapté a la demande faite.

\subsubsection{Token Lifecycle Management}
Le Token Service Provider \textbf{DOIT} mettre en place des moyens de gerer le cycle de vie d'un Token emis. Ces interfaces peuvent etres utilisées par de nombreuses entités telles que le réseau de paiement, le Token Requestor, la banque emettrice...
L'implémentation de ces interfaces sont laissés a la discretion du Token Service Provider.
